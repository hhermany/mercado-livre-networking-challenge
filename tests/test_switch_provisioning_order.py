...
def test_domain_is_mercadolivre():
    ...
    assert "ip domain name MercadoLivre.local" in config

    assert "unimed" not in config.lower()


def test_required_aaa_block_precedes_dot1x_interfaces():
    config = build_candidate()

    aaa_new_model = config.index("aaa new-model")

    radius_group = config.index("aaa group server radius RAD")

    aaa_login = config.index("aaa authentication login default local")

    aaa_dot1x = config.index("aaa authentication dot1x default group RAD")

    aaa_authorization = config.index("aaa authorization network default group RAD")

    aaa_accounting = config.index("aaa accounting dot1x default start-stop group RAD")

    coa = config.index("aaa server radius dynamic-author")

    dot1x_global = config.index("dot1x system-auth-control")

    user_interface = config.index("interface range Gi0/1 - 2")

    interface_auth = config.index(" authentication open")
...
def test_radius_source_matches_management_vlan():
    ...
    assert "ip radius source-interface Vlan255" in config


def test_radius_source_follows_user_interfaces_in_candidate():
    config = build_candidate()

    users = config.index("interface range Gi0/1 - 2")

    source = config.index("ip radius source-interface Vlan255")
...
def test_admin_login_uses_local_default():
    ...
    assert "aaa authentication login default group RAD" not in config

    assert "LOCAL-ADMIN" not in config
...
def test_radius_server_definitions_use_rad1_rad2():
    ...
    assert "radius server RAD2" in config

    assert "radius server ISE" not in config

    assert "radius server ISE2" not in config
