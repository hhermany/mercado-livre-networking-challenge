...
def test_aaa_minimum_contract_is_present():
    ...
    required = [
        ...
        (" client 192.168.0.178 server-key 7 13061E010803"),
        (" client 192.168.0.177 server-key 7 0822455D0A16"),
...
def test_required_aaa_sequence_is_preserved():
    ...
    positions = [config.index(command) for command in commands]
...
def test_radius_names_and_keys_are_fixed():
    ...
    assert "key 7 094F471A1A0A" in config

    assert "key 7 070C285F4D06" in config
